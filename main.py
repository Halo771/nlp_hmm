...
from flask import Flask, render_template, request, session, send_file, make_response,redirect,jsonify
import os
import uuid
...
def get_response_from_ChatGPT_API(message_context, apikey,
                                  model="chatglm2-6b", temperature=0.9, presence_penalty=0.8, max_tokens=2000):
    """
    从ChatGPT API获取回复
...
def get_response_stream_generate_from_ChatGPT_API(message_context, apikey, message_history,
                                                  model="chatglm2-6b", temperature=0.9, presence_penalty=0.9,
                                                  max_tokens=2000):
    """
    从ChatGPT API获取回复
    :param apikey:
    ...
    header = {"Content-Type": "application/json",
              "Authorization": "Bearer " + apikey}
    que=message_context[-1]["content"]
    result0 = search_car_models_in_sentence(que)
    if len(result0) == 1:
        system=se_tu_guzhang(result0[0],que)
        kgmessage= {'role': 'system', 'content':system }
    #kgmessage= {'role': 'system', 'content': '根据公安部道路交通事故统计报告，我国每年大约有10％左右的交通事故直接与雨雪雾等恶劣天气有关。高速公路由于技术等级高、设施完善、控制出入等特点，车辆行驶速度高，因此，大雾诱发的高速公路交通事故往往是灾难性的，造成重大人员伤亡。据统计，大雾已成为导致高速公路连环相撞恶性交通事故的第一杀手，这方面的案例不在少数。这也是当前高速公路一旦出现雾，必须采取限速等交通管制措施，甚至是封闭道路的原因。'}
        message_context.insert(-1, kgmessage)
    else:
        print("没有匹配到")
    data = {
        "model": model,
...
def download_user_dict_file():
    ...
    if request.headers.get("admin-password") is None:
        ...
        #print(request.headers.get("admin-password"),ADMIN_PASSWORD)
...
def login ():
    if request.method =="GET":
        ...
        if username == "admin" and password == "abc":
            #return HttpResponse("登陆成功")http://192.168.237.33:5000/
            return redirect("http://127.0.0.1:5000/answer/")
        else:
            return render_template("login.html",error_message="用户名或者密码错误")

def echartspic1():
    
    graph = Graph("http://49.232.134.66:7474", auth=("neo4j", "666666")) 
    n0="MATCH p=(n)-[r]->(m) RETURN id(n),n.name,labels(n),id(m),m.name,labels(m),type(r) LIMIT 25"
    n1='MATCH p=(n)-[r:aquaculture]->(m) RETURN id(n),n.name,labels(n),id(m),m.name,labels(m),type(r) LIMIT 25'
    translations={'Equipment_module':"设备模块",'Use_condition':'使用条件','Comprise':'模块内容','Main_frame':'主要框架','use_condition':'使用条件为','comprise':'拥有模块','mainframe':'拥有框架',"aquaculture":'海洋牧场装备','Equip_name':'设备名称',"Aquaculture":'海洋牧场大型装备'}
    data0 = graph.run(n0).data()
    data1=graph.run(n1).data()
    #data2=graph.run(n2).data()
    # 打印新的列表
    #print(data0)
    #python怎么从下面列表中提取车型等相应数据
    # 获取车型数据
    nodes=[]
    relations=[]
    node={
                ...
                'name':data0[0]['n.name'],
                'symbolSize': 60,
                'category':translations[data0[0]['labels(n)'][0]]
            }
    nodes.append(node)
    for rel in data0:
        node = {
                ...
                'name':rel["m.name"],
                'symbolSize': 60,
                'category': translations[rel['labels(m)'][0]]
            }
        relation={
            'source': str(data0[0]['id(n)']),
            'target':str(rel['id(m)']),
            'name':translations[rel['type(r)']]
            }
        nodes.append(node)
    ...
    for rel in data1:
        node1 = {
                ...
                'name':rel["n.name"],
                'symbolSize': 60,
                'category': translations[rel['labels(n)'][0]]
            }
        node2 = {
                'id': str(rel['id(m)']),
                'name':rel["m.name"],
    ...
    for dictionary in nodes:
        if dictionary not in new_list:
            new_list.append(dictionary)
    return(new_list,relations)

def echartspic2():
    
    graph = Graph("http://49.232.134.66:7474", auth=("neo4j", "666666")) 
    n0="MATCH p=(n)-[r]->(m) RETURN id(n),n.name,labels(n),id(m),m.name,labels(m),type(r) LIMIT 5"
    n1='MATCH p=(n)-[r:aquaculture]->(m) RETURN id(n),n.name,labels(n),id(m),m.name,labels(m),type(r) LIMIT 5'
    translations={'Equipment_module':"设备模块",'Use_condition':'使用条件','Comprise':'模块内容','Main_frame':'主要框架','use_condition':'使用条件为','comprise':'拥有模块','mainframe':'拥有框架',"aquaculture":'海洋牧场装备','Equip_name':'设备名称',"Aquaculture":'海洋牧场大型装备'}
    data0 = graph.run(n0).data()
    data1=graph.run(n1).data()
    #data2=graph.run(n2).data()
...
@app.route('/echartspic/',methods=['GET', 'POST'])
def echartspic():
    data,links=echartspic1()
    search_neo4j_data = {
            'data': data,
            'links': links
        }
...
#print(search_neo4j_data)
    ...
    return render_template('echartspic.html',neo4j_data=search_neo4j_data)


@app.route('/your_data_endpoint/')
def get_new_data():
    # 在这里编写获取新数据的代码，假设新数据存储在new_data变量中
    data,links=echartspic2()
    search_neo4j_data = {
            'data': data,
            'links': links
        }
    ...
    #print(len(search_neo4j_data["data"]))
    search_neo4j_data = json.dumps(search_neo4j_data)
    return search_neo4j_data


@app.route('/your_data_endpoint1/')
def get_new_data1():
    time.sleep(5)
    # 在这里编写获取新数据的代码，假设新数据存储在new_data变量中
...
def answer():
    ...
    chexing="元PLUS"
    nodes,relations,sentences,contents=se_tu(chexing)
    ...
    return render_template('answer.html', neo4j_data=search_neo4j_data)
...
def se_tu(chexing):
    ...
    graph = Graph("http://localhost:7474", auth=("neo4j", "20010102SBY"),name='test')
    n0="MATCH p=(n:`投诉车型` {`投诉车型`:'"+chexing+"'})-[:`被投诉`]-()-[:`存在`]-(m) return n,id(n),count(m)"
...
# 获取车型名称、投诉品牌和价格区间
    ...
    for dictionary in nodes:
        if dictionary not in new_list:
            new_list.append(dictionary)
    return(new_list,relations,sentences,contents)
